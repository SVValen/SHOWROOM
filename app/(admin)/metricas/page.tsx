...
export default function MetricasPage() {
  ...
  if (!metricas) {
    return <div className="p-8 text-red-500 text-sm">Error al cargar métricas.</div>
  }
    ...
    <div className="p-4 sm:p-6 lg:p-8 max-w-5xl mx-auto">
      ...
      <Section title="Inventario actual">
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          <MetricCard label="Productos disponibles" value={metricas.productos_disponibles.toString()} sub={`de ${metricas.total_productos} totales`} />
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-4">
      ...
      <p className={`text-xl sm:text-2xl font-bold leading-none ${
        red ? 'text-red-500' : green ? 'text-emerald-600' : 'text-gray-900'
      }`}>
        {value}
      </p>
